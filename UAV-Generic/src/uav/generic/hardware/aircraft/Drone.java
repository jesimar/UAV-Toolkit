...
public abstract class Drone {
    ...
    double SPEED_MAX;//in m/s
    double SPEED_CRUIZE;//in m/s
    double MASS;//in kg
    ...
    public String title(){
        return "time;lat;lng;alt_rel;alt_abs;voltage_bat;current_bat;level_bat;pitch;yaw;roll;"
                + "vx;vy;vz;fixtype;satellitesvisible;eph;epv;heading;groundspeed;airspeed;"
                + "next_wpt;count_wpt;dist_to_home;dist_to_current_wpt;mode;system-status;"
                + "armed;is-armable;ekf-ok";
    }     
        
    @Override
    public String toString() {
        return String.format("%.1f;%.7f;%.7f;%.2f;%.2f;%.3f;%.2f;%.1f;%.4f;%.4f;%.4f;%.2f;"
                + "%.2f;%.2f;%d;%d;%d;%d;%.1f;%.2f;%.2f;%d;%d;%.2f;%.2f;%s;%s;%s;%s;%s", 
                time, gps.lat, gps.lng, barometer.alt_rel, barometer.alt_abs,
