...
## Desenvolver sistema para Escolha da missão a ser executada.
...
* Melhorar a calibração do power module no piloto automático.
...
* Incluir verificação de aeronaves intrusas incluindo informações como número de aeronaves intrusas, rotas percorridas por tais aeronaves, distância da aeronave intrusa mais próxima, projeção futura da aeroanve para verificar chance de colisão, conforme descrito em Mattei 2015.
* Abrir o paraquedas somente se der 2Dfix ou 1Dfix ou 0Dfix, por mais de 2 segundos.
* Deve verificar a rota antes de enviar para o PA. Verificar se existe mais de um comando de TAKEOFF. Só pode haver um comando desse tipo.
* Deve verificar se existe mais de um comando do tipo LAND, LAND_VERTICAL ou RTL. Só pode haver um comando desse tipo.
* Deve ter um objeto com todos os waypoints da rota.
* Discutir uma forma fácil de fazer a projeção da aeronave no futuro após ocorrer a falha crítica.
* Deve encerrar sua execução quando o drone levantar o voo e depois pousar.
...
* Adicionar campos no arquivo de log como ligou led, bateu foto, iniciou vídeo, buzzer apitou, ligou alarme, abriu paraquedas, etc (dificil de fazer, pois algumas informações estão disponível apenas no MOSA).
* Melhorar o código evitando acoplamento e coesão do código em Java (trabalhar com interfaces e abstrações).
* Recebe a missão via socket e retorna uma mensagem se aceitou ou não a rota (motivo)
...
* Deve encerrar sua execução quando o drone levantar o voo e depois pousar.
* Deve ter um objeto com todos os waypoints da rota.
* Deve aguardar até que o modo seja STANDBY, antes disso nao adianta calcular nada.
* Deve chamar as rotas standard a partir do ponto onde o drone está localizado.
* Definir melhor o home da missão e o launch da missão.
* Melhorar o código evitando acoplamento e coesão do código em Java (trabalhar com interfaces e abstrações).
* Deve atua direto na câmera (sem perguntar nada ao IFA)
* Não atuar direto com a missão (todas as informações devem ser passadas ao IFA)
* Tomar cuidado com o Exit(0), pois ele precisa avisar ao IFA antes de sair.
* Integrar o módulo do HISA4m (Márcio).
* Integrar o módulo do CSA4m (Márcio).
* Integrar o módulo do HCCQSP (Márcio).
* Incluir planejador baseado em Campos Potenciais.
* Incluir planejador baseado em A*.
...
* Aplicação básica na Edison e Raspberry Pi para efetuar o disparo do paraquedas.
